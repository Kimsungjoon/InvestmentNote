...
★ 눌림목 모드: 5MA 근처 눌림 + RSI 40~70 + 이격 ≤4%
☆ 모멘텀 모드: 5MA 가속(+4~15%) + RSI 55~80 + 신고가 근처 + 거래량 + RS>QQQ
"""
...
# ── 모멘텀 모드 (듀얼 출력) ──
MIN_EXT_5MA_MOM = 4.0    # 5일선 이격 하한 — 5MA 위 가속 구간
MAX_EXT_5MA_MOM = 15.0   # 5일선 이격 상한 — 과열 추격 차단
MOM_RSI_MIN     = 55     # 모멘텀 RSI 하한 (강세 확인)
MOM_RSI_MAX     = 80     # 모멘텀 RSI 상한
...
def analyze(ticker: str, name: str, marketcap: float | None = None,
    ...
    momentum_grade = (trend_ok and short_ok and above_5ma and mom_ext_ok
                      and mom_rsi_ok and mom_near_high and mom_vol_ok
                      and checks["MACD 히스토 상승"] and rs_ok
    ...
    return {
        ...
        "from_high": from_high, "ext_5ma": ext_5ma, "ext_ok": ext_ok,
...
def exclusion_tag(a: dict, market_ok: bool) -> str:
    ...
    if not a["checks"]["주가>5일선"]:
        return "5MA아래"
    if not a.get("ext_ok", True) and a["ext_5ma"] > MAX_EXT_5MA:
        return "이격↑"
...
def main():
    ...
    print(f"  [눌림목] 시총 ${MIN_MARKETCAP_B}~${MAX_MARKETCAP_B}B | 단가≤${MAX_PRICE} | "
          f"RSI {RSI_MIN}~{RSI_MAX} | 5MA이격≤{MAX_EXT_5MA}%")
    print(f"  [모멘텀] 5MA이격 {MIN_EXT_5MA_MOM}~{MAX_EXT_5MA_MOM}% | RSI {MOM_RSI_MIN}~{MOM_RSI_MAX} | "
          f"고점대비≥{MOM_FROM_HIGH}% | 거래량≥{MOM_VOLUME_MULT}x | RS>QQQ")
    print("=" * 78)
    ...
    print(f"\n{'='*78}")
    print(f"  ☆ 모멘텀 적격 (5MA이격 {MIN_EXT_5MA_MOM}~{MAX_EXT_5MA_MOM}%, "
          f"RSI {MOM_RSI_MIN}~{MOM_RSI_MAX}, 거래량·RS): {len(momentums)}개")
    print("=" * 78)
    for a in momentums:
